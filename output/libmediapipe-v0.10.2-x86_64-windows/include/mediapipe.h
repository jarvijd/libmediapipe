...
extern "C" {
    ...
    typedef struct {
        mp_handedness* elements;
        int length;
    } mp_handedness_list;

    typedef struct {
        mp_handedness_list* elements;
        int length;
    } mp_multi_face_handedness_list;
    ...
    /// Returns NULL on failure.
    MEDIAPIPE_API mp_poller* mp_create_poller(mp_instance* instance, const char* output_stream, bool observe_timestamp_bounds = false);
    ...
    /// Sends a packet to the GPU input stream specified in mp_create_instance_builder.
    /// The results won't be available immediately. Call mp_get_queue_size to check whether the
    /// packet has been processed or mp_wait_until_idle to block until the results are available.
    /// Returns true on success.
    ...
    MEDIAPIPE_API void mp_destroy_landmarks(mp_landmark_list* list);
    ...
    MEDIAPIPE_API void mp_destroy_handedness(mp_multi_face_handedness_list* list);
